...
// ─────────────────────────────────────────────────────────────────────────────
// Anomaly Detection (Cosine Similarity)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Image Storage: save evidence image to logs/YYYY/MM/DD/
// ─────────────────────────────────────────────────────────────────────────────
...
export const handleDetection = async (req: Request, res: Response) => {
  try {
    const {
      ...
      camera_id,
      image_path,
      image_base64,        // Optional: base64 JPEG for evidence storage
      vehicle_type,
      vehicle_color,
      vehicle_fingerprint,
      processing_time_ms,  // NEW
      image_quality_score, // NEW
      track_id,            // NEW
    } = req.body;
    ...
    if (lastSeen && (now - lastSeen) < COOLDOWN_TIME_MS) {
      console.log(`[Cooldown] Ignored ${plate_number} (${Math.round((now - lastSeen) / 1000)}s ago)`);
      res.status(200).json({ success: true, message: 'Ignored due to cooldown' });
      return;
    }
    cooldownCache.set(plate_number, now);

    // Direction: camera 1 = IN, camera 2 = OUT
    const direction = (camera_id === 2 || camera_id === '2') ? 'OUT' : 'IN';
    ...
    console.log(`[Detection] ${resolvedPlate} | ${vehicleTypeLabel} | ${direction}` +
                (isFuzzyMatch ? ' [FUZZY]' : '') +
                (track_id ? ` | ${track_id}` : ''));
    ...
    // ── image_expire_at = NOW + RETENTION_DAYS ────────────────────────────
    ...
    const [insertResult]: any = await pool.query(
      `INSERT INTO vehicle_logs
         (plate_number, vehicle_id, camera_id, direction,
          confidence_score, image_path, image_expire_at,
          is_fuzzy_match, original_ocr_text,
          processing_time_ms, image_quality_score, track_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        resolvedPlate,
        vehicleId,
    ...
    if (io) {
      ...
      // Stats update (throttled — emit only on detection)
      try {
  ...
  try {
    ...
    const [rows]: any = await pool.query(`
      SELECT vl.id, vl.plate_number as plate, vl.direction as dir,
             vl.created_at as time, vl.confidence_score, vl.image_quality_score,
